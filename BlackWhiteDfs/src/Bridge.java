public class Bridge {
  ...
  private void dfs(UndirectedGraph G, int cur, int prev) {
    ...
    System.out.println(" erliestVisitedReachable: " + erliestVisitedReachable[cur]
        + " dfsVisitedOrder: " + dfsVisitedOrder[cur]);
    for (int w : G.adj(cur)) {
      ...
      if (dfsVisitedOrder[w] == -1) { // not visited
        ...
        erliestVisitedReachable[cur] =
            Math.min(erliestVisitedReachable[cur], erliestVisitedReachable[w]);
        System.out.println("Changing1:  erliestVisitedReachable: " + erliestVisitedReachable[cur]
            + " dfsVisitedOrder: " + dfsVisitedOrder[cur]);


        // If the lowest vertex reachable from subtree under v is
        // below u in DFS tree, then u-v is a bridge
        if (dfsVisitedOrder[cur] < erliestVisitedReachable[w]) { // if (erliestVisitedReachable[w]
                                                                 // == dfsVisitedOrder[w])
      ...
      // ignore reverse of edge leading to cur
        ...
        System.out.println("Changing2:  erliestVisitedReachable: " + erliestVisitedReachable[cur]
            + " dfsVisitedOrder: " + dfsVisitedOrder[cur]);
      }
    }
